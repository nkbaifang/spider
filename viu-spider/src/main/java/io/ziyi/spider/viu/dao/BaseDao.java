...
import io.ziyi.spider.viu.common.Page;
...
public abstract class BaseDao extends BaseComponent {
    ...
    protected <K extends Serializable, E extends BaseModel<K>> E find(Class<E> type, K key) {
        if ( key == null ) {
            return null;
        }
        EntityManager entityManager = getEntityManager();
    ...
    protected <K extends Serializable, E extends BaseModel<K>> E find(Class<E> type, K key, boolean forUpdate) {
        if ( key == null ) {
            return null;
        }
        EntityManager entityManager = getEntityManager();
        LockModeType lockModeType = forUpdate ? LockModeType.PESSIMISTIC_WRITE : LockModeType.PESSIMISTIC_READ;
        return entityManager.find(type, key, lockModeType);
    }
